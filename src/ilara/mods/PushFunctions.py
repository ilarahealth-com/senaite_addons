...
class IlaraFunctions(object):
    ...
    def returnBills(self):
        ...
        aRequest_query_results = api.search(
            {"portal_type": "AnalysisRequest", "Complete": True}
        )
    ...
    def returnInvoiceAmount(self, sample_id):
        ...
        aRequest_query_results = api.search(
            {"portal_type": "AnalysisRequest", "id": "%s" % sample_id, "Complete": True}
        )
        ...
        if len(aRequest_query_results) > 0:
            ...
            logger.info(
                "Analysis Request Results: {0}".format(len(aRequest_query_results))
            )
            ...
            try:
                ...
                logger.info(
                    "{0}: {1}: {2}".format(sample.Title(), sample.getSubtotal())
                )
            except Exception as e:
                logger.info("Failed to get sample object properties {0}".format(e))
            ...
            aRequest_cat_query_results = api.search(
                {"id": "%s" % sample_id}, catalog="bika_catalog_analysisrequest_listing"
            )
            logger.info(
                "Analysis Request Catalog Results: {0}".format(
                    len(aRequest_cat_query_results)
                )
            )
            ...
            patient_query_results = api.search(
                {"portal_type": "Patient", "id": "%s" % patient_id, "Complete": True}
            )
            ...
            try:
                ...
                logger.info(
                    "{0} - {1}".format(patient.getFullname(), patient.getMobilePhone())
                )
            except Exception as e:
                logger.info("Failed to get patient object properties {0}".format(e))

            # Getting creator
            LabContacts_query_results = api.search(
                {"portal_type": "LabContact", "Complete": True}
            )

            try:
                LabContacts = map(api.get_object, LabContacts_query_results)
                filtered = filter(
                    lambda contact: contact.EmailAddress == creator_email, LabContacts
                )
